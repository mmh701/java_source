...
public class Ex1Car { // Car 객체(object)를 생산하기 위한 설계도 : 클래스
	...
	// field(필드, 속성과 method(행위)를 멤버로 가질 수 있다.
	...
	void def() { // 메소드 수행 후 반환값이 없이 돌아갈 경우는 void
		...
		System.out.println("d에어백 수는 " + airBag); // private 필드이지만 현재 클래스내에서 호출 가능
		abc(); // private 필드이지만 현재 클래스내에서 호출 가능
	}
	...
	// 1) 변수 = 값     ex: a = 5
	// 2) 매개변수를 이용.     메소드명(타입 매개변수){...} 하고 메소드명(값) 
