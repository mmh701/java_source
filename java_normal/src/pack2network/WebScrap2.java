...
// jsoup 은 실제 HTML 및 XML 작업을 간소화하는 Java 라이브러리입니다.
...
public class WebScrap2 {
	...
	public static void main(String[] args) {
		...
		// URL 인코딩 (또는 퍼센트 인코딩)은 특수 문자와 기타 예약된 문자들을 웹 주소 (URL)에서 
		...
		try {
			// System.out.println(URLEncoder.encode("비욘세", "utf-8")); // %EB%B9%84%EC%9A%98%EC%84%B8
	...
	private static void printKorText(String text) {
		// 정규 표현식 사용
		// 프로그래밍에서 문자열을 다룰 때, 문자열의 일정한 패턴을 표현하는 일종의 현식 언어를 말한다.
		Pattern pattern = Pattern.compile("[가-힣\\s]+"); // 정규 표현식 패턴 작성
		Matcher matcher = pattern.matcher(text);
		while(matcher.find()) {
			String line = matcher.group().trim();	// 앞뒤 공백 제거
			if(!line.isEmpty()) {	// 빈 줄이 아닌 경우에만 출력
