...
public class MyStream1 {
	...
	private void test1() {
		// Stream API를 사용하려면 Stream 생성해야 함
	...
	private void test2() {
		...
		Iterator<String> iter = list.iterator();	// 전통적 방법 : 외부 반복자 사용
		while(iter.hasNext()) {
		...
		// 작업으로 전달. 다양한 작업을 한 번의 스트림에서 연속적으로 체이닝하여 수행할 경우에 사용함.
	...
	private void test3() {
		...
		double avg = slist.stream()
						.mapToInt(Student :: getAge) //  Student 객체를 age값으로 매핑 age로 스트림 생성
						.average()	// age 요소의 평균을 OptionalDouble 타입(null 관련 에러 회피)으로 저장
						.getAsDouble();
		...
		double avgResult = slist.stream()
								...
								.average()
								.getAsDouble();
		System.out.println("학생 나이 평균 : " + avgResult);
		
		slist.stream()
		.filter(ir -> ir.getName().startsWith("한"))
